...
#Region " Constructor "
    ...
    Public Sub New( _
            ...
            ByVal agentHost As String, _
            ByVal agentNumber As Integer)
...
#Region " Xabsl Commands "
        ...
        SyncLock Me._XabslConnection
            ...
            Try
                ...
                If waitForReply Then
                    'block this thread until acknowledgment or failure message is received
    ...
    Protected Function RegisterAgent() As Boolean
        ...
        Try
            ...
            Dim reply As String = ""
            Dim success As Boolean = Me.SendXabslCommand(String.Format("INIT {{Robot {0}}} {{Port {1}}}", Me._AgentName, Me._ListenerPort), True, reply)

            ' Xabsl should return a message in the form of {Status OK <message>}
...
#Region " Device Lifetime "
    ...
    Public Sub StartDevice() Implements ICommDevice.StartDevice
        ...
        'Xabsl should be running a server right now.

        'First connect to the Xabsl server
        Me.ConnectToXabsl()

        'Now register this agent to Xabsl
        While Not Me.RegisterAgent()
            MsgBox("Registering Agent failed. Perhaps start XABSL?", MsgBoxStyle.OkOnly, "Registration failure")
            'Retry connecting to xabsl
            Me.ConnectToXabsl()
        End While
        ' Then start a listener for Xabsl to connect to
    ...
    Protected Overrides Sub Run()
        ...
        While Me.IsRunning
            ...
            Console.WriteLine("running XabslDevice")
...
#End Region
    ...
    ' I dont think I'll need conversations right now, but they are needed for the interface, so I'll just let them stay here.
