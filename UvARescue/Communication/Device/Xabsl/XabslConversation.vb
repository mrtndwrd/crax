...
Public Class XabslConversation
        ...
        Try
            While Me.IsRunning AndAlso Me.IsConnected
                If Not Me.Receive(4096) Then
                    ...
                    If Me.ConversationCam Then
                        'Thread.Sleep(TimeSpan.FromMilliseconds(700)) 'slightly different from sensor-thread
                        Thread.Sleep(TimeSpan.FromMilliseconds(155)) 'slightly different from sensor-thread
                        'Thread.Sleep(TimeSpan.FromMilliseconds(37)) 'slightly different from sensor-thread
                        'Thread.Sleep(TimeSpan.FromMilliseconds(377)) 'slightly different from sensor-thread
                    Else
                        ...
                        Thread.Sleep(TimeSpan.FromMilliseconds(133)) 'slightly different from camera-thread
...
#Region " Core Data Sending and Retrieval Routines "
    ...
    Protected Sub Send(ByVal protocol As Protocol, ByVal data() As Byte)
        ...
        System.Buffer.BlockCopy(data, 0, bytes, offset + infolen, data.Length)
...
#End If
    ...
    Protected Function Receive(ByVal maxLength As Integer) As Boolean
        ...
        If Me.IsDataAvailable Then
            ...
            System.Buffer.BlockCopy(buffer, 0, msgbuffer, offset, nread)
#If ORIGINAL Then
...
#End If
        ...
        If Not IsNothing(Me.msgbuffer) AndAlso Me.msgbuffer.Length > 5 Then
            ...
            If msgbuffer.Length >= msglength + 5 Then
                ...
                System.Buffer.BlockCopy(msgbuffer, 5, curmsg, 0, curmsg.Length)
#If ORIGINAL Then
...
#End If
                ...
                System.Buffer.BlockCopy(msgbuffer, curmsg.Length + 5, nxtmsg, 0, nxtmsg.Length)
#If ORIGINAL Then
    ...
    Protected Sub SendMessage(ByVal message As Message) Implements ICommConversation.SendMessage
        ...
        Try
            ...
            Using stream As New MemoryStream
                ...
                End If
                'Not needed, see http://www.digitalcoding.com/Code-Snippets/VB/Visual-Basic-Code-Snippet-Save-object-to-file.html
                'formatter = Nothing
